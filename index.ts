...
const filtered = JSON.stringify(
  data.map((item) => {
    return {
      name: item.name,
      level: item.maxLevel,
      isEvolution: item.maxEvolutionLevel ? true : false,
      evolutionLevel: item.maxEvolutionLevel ? item.maxEvolutionLevel : null,
      rarity: item.rarity,
    };
  })
);

await Bun.write("./model/demo.json", filtered);
